...
export const DispatchDrawer: React.FC<DispatchDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#041122] border-l border-[#0033A0]/60 w-full max-w-xl h-full flex flex-col justify-between shadow-2xl overflow-hidden animate-in slide-in-from-right duration-300">
        
        {/* Top Header */}
        <div className="p-5 border-b border-[#0033A0]/40 bg-[#071B38]">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2.5">
              <div className="relative">
                <span className="w-3 h-3 rounded-full bg-[#FF5500] block" />
                <span className="w-3 h-3 rounded-full bg-[#FF5500] animate-ping absolute inset-0 opacity-75" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={onRefresh}
                className="p-2 rounded-xl bg-[#041122] hover:bg-[#0C244C] text-slate-300 hover:text-white border border-[#0033A0]/50 transition-colors cursor-pointer"
          ...
          <div className="grid grid-cols-3 gap-1.5 p-1 bg-[#020813] rounded-xl border border-[#0033A0]/50">
            <button
        ...
        {activeTab === 'chat' && (
          <div className="flex-1 flex flex-col justify-between overflow-hidden bg-[#020813]">
            {/* Quick Dispatch Banner */}
            <div className="px-5 py-2.5 bg-[#071B38] border-b border-[#0033A0]/40 flex items-center justify-between text-xs">
              ...
              <a
                ...
                className="text-[#FF7700] hover:text-white font-bold underline flex items-center gap-1"
              >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
                ...
                >
                  ...
                  <div
                    className={`max-w-[85%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed shadow-lg ${
                      msg.sender === 'user'
                        ? 'bg-gradient-to-r from-[#FF7700] via-[#FF5500] to-[#E63900] text-white font-bold rounded-br-none'
                        : 'bg-[#071B38] text-slate-200 border border-[#0033A0]/60 rounded-bl-none'
                    }`}
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2.5 max-w-[90%]">
                      {msg.suggestedActions.map((action, idx) => (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-[#071B38] hover:bg-[#0C244C] text-white text-[11px] font-semibold border border-[#0033A0]/60 transition-all cursor-pointer flex items-center gap-1"
                        >
              ...
              {isTyping && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-[#071B38] border border-[#0033A0]/50 max-w-[140px]">
                  <span className="text-xs text-slate-400">Dispatcher typing</span>
                  <span className="flex gap-1">
                    <span className="w-1.5 h-1.5 bg-[#FF5500] rounded-full animate-bounce" />
                    <span className="w-1.5 h-1.5 bg-[#FF5500] rounded-full animate-bounce [animation-delay:0.2s]" />
                    <span className="w-1.5 h-1.5 bg-[#FF5500] rounded-full animate-bounce [animation-delay:0.4s]" />
                  </span>
                </div>
              )}
            ...
            {/* Chat Input Box */}
            <div className="p-4 border-t border-[#0033A0]/40 bg-[#071B38]">
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleSendMessage();
              ...
              >
                <input
                  ...
                  className="flex-1 bg-[#041122] border border-slate-700 rounded-xl px-4 py-3 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-[#FF5500] transition-colors"
                />
                <button
                  type="submit"
                  disabled={!inputMessage.trim()}
                  className="p-3 rounded-xl bg-gradient-to-r from-[#FF7700] via-[#FF5500] to-[#E63900] hover:from-[#FFA040] hover:to-[#FF5500] disabled:opacity-50 text-white font-bold transition-all cursor-pointer shadow-md shadow-[#FF5500]/20"
                >
              ...
              <div className="flex items-center justify-between mt-2.5 px-1 text-[11px] text-slate-400">
                ...
                <button
                  onClick={() => setActiveTab('call')}
                  className="text-[#FF7700] hover:underline flex items-center gap-1 font-bold cursor-pointer"
                >
                  <Phone className="w-3 h-3" />
        ...
        {activeTab === 'call' && (
          <div className="flex-1 overflow-y-auto p-6 bg-[#020813] space-y-6">
            
            {/* Primary Tap to Call Card */}
            <div className="p-6 rounded-2xl bg-gradient-to-b from-[#071B38] to-[#041122] border border-[#0033A0]/60 text-center relative overflow-hidden shadow-2xl">
              <div className="w-16 h-16 rounded-full bg-[#FF5500]/20 border border-[#FF5500]/40 flex items-center justify-center mx-auto mb-4 text-[#FF5500] shadow-lg shadow-[#FF5500]/20">
              ...
              <p className="text-slate-300 text-xs mb-6 max-w-sm mx-auto">
            ...
            <div className="p-4 rounded-xl bg-[#071B38] border border-[#0033A0]/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-xl bg-[#041122] text-[#FF5500] border border-[#0033A0]/50">
                  <Volume2 className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-white text-xs font-bold">Edmonton Dispatch Voice Bridge</h4>
                  <p className="text-slate-300 text-[11px]">
                    Average wait time: <span className="text-[#FF5500] font-semibold">&lt; 15 seconds</span> • 3 Dispatchers on Duty
                  </p>
                </div>
              </div>
              <span className="px-2.5 py-1 rounded-md bg-[#0033A0]/40 text-white border border-[#0033A0] text-[10px] font-bold uppercase">
                Active Line
              </span>
            </div>

            {/* Request Instant Call Back Form */}
            <div className="p-6 rounded-2xl bg-[#071B38] border border-[#0033A0]/50">
              <div className="flex items-center gap-2 text-[#FF5500] font-bold text-xs uppercase tracking-wider mb-2">
              ...
              {callbackStatus === 'success' ? (
                <div className="p-4 rounded-xl bg-[#041122] border border-[#FF5500]/40 text-center space-y-2">
                  <CheckCircle2 className="w-6 h-6 text-[#FF5500] mx-auto" />
                  <h4 className="text-white font-bold text-sm">Call Request Sent to Dispatch!</h4>
                  <p className="text-slate-300 text-xs">{callbackMessage}</p>
                  <button
                    onClick={() => setCallbackStatus('idle')}
                    className="mt-2 text-xs text-[#FF5500] font-bold underline cursor-pointer"
                  >
                ...
                <form onSubmit={handleRequestCallback} className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[#041122] border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-[#FF5500]"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-slate-300 mb-1">
                        Your Phone Number <span className="text-[#FF5500]">*</span>
                      </label>
                      <input
                        type="tel"
                        required
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#FF7700] via-[#FF5500] to-[#E63900] hover:from-[#FFA040] hover:to-[#FF5500] text-white font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-[#FF5500]/30 cursor-pointer disabled:opacity-50 border border-orange-300/40"
                  >
        ...
        {activeTab === 'feed' && (
          <div className="flex-1 overflow-y-auto p-5 bg-[#020813] space-y-3">
            {/* Dispatch Channel Routing Summary */}
            <div className="p-4 rounded-xl bg-[#071B38] border border-[#0033A0]/50 space-y-2 text-xs">
              <div className="flex items-center justify-between text-slate-300">
                <span className="flex items-center gap-1.5 text-white font-semibold">
                  <Phone className="w-3.5 h-3.5 text-[#FF5500]" />
                  <span>Primary SMS Dispatch:</span>
                </span>
                <span className="font-mono text-[#FF5500] font-bold">+1 (587) 728-2828</span>
              </div>
            ...
            <h4 className="text-xs uppercase font-bold tracking-wider text-white pt-2">
              Recent Chauffeur Dispatches ({recentBookings.length})
            </h4>

            {recentBookings.length === 0 ? (
              <div className="text-center py-12 text-slate-400 text-xs bg-[#071B38] rounded-xl border border-[#0033A0]/50">
              ...
              recentBookings.map((b) => (
                <div
                  key={b.id}
                  className="p-4 rounded-xl bg-[#071B38] border border-[#0033A0]/50 hover:border-[#FF5500]/60 transition-colors text-xs space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-cinzel font-bold text-[#FF5500]">
                      {b.id}
                    </span>
                    <span className="text-[10px] bg-[#0033A0] text-white px-2 py-0.5 rounded font-bold">
                      Dispatched
                    </span>
                  </div>
                  ...
                  <div className="text-slate-400 text-[11px] space-y-0.5">
                    ...
                    <div className="flex items-center gap-1">
                      <MapPin className="w-3 h-3 text-[#FF7700]" />
                      <span className="truncate">To: {b.destination}</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-[11px] pt-2 border-t border-[#0033A0]/40 text-slate-300">
        ...
        <div className="p-4 border-t border-[#0033A0]/40 bg-[#071B38] flex items-center justify-between gap-3">
          <a
            href="tel:+15877282828"
            className="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-[#FF7700] via-[#FF5500] to-[#E63900] hover:from-[#FFA040] hover:to-[#FF5500] text-white font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-[#FF5500]/30 transition-all border border-orange-300/40"
          >
            <Phone className="w-4 h-4 fill-current" />
          ...
          <button
            ...
            className="flex-1 py-3 px-4 rounded-xl bg-[#041122] hover:bg-[#0C244C] border border-[#0033A0]/60 text-white font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
          >
