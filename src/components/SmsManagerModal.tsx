...
export const SmsManagerModal: React.FC<SmsManagerModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#101015] border-2 border-amber-500/40 rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="p-6 border-b border-stone-800 bg-[#14141a] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
                  Live System
                </span>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="px-3 py-1.5 rounded-xl bg-stone-900 hover:bg-stone-800 text-stone-400 hover:text-white text-xs font-semibold border border-stone-800 transition-colors cursor-pointer"
            >
              Close
            </button>
          </div>
        ...
        {/* Tab Controls */}
        <div className="px-6 pt-4 pb-2 bg-[#121217] border-b border-stone-800 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <button
          ...
          <div className="text-[11px] text-stone-400 flex items-center gap-2">
            ...
            <span className={`px-2 py-0.5 rounded font-mono font-semibold ${hasTwilio ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-300'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === 'audit' && (
            <div className="space-y-4">
              ...
              {bookings.length === 0 ? (
                <div className="text-center py-12 bg-[#14141a] rounded-2xl border border-stone-800 text-stone-400 text-xs">
                ...
                <div className="space-y-3">
                  {bookings.map((booking) => {
                    ...
                    return (
                      <div
                        key={booking.id}
                        className="p-4 rounded-2xl bg-[#14141a] border border-stone-800 hover:border-stone-700 transition-all space-y-3 text-xs"
                      >
                        {/* Header line */}
                        <div className="flex items-start justify-between gap-2 flex-wrap">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-stone-400">({booking.phone})</span>
                          </div>
                        ...
                        {/* Route & Ride summary */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-stone-300 bg-black/40 p-2.5 rounded-xl border border-stone-800/80">
                          <div>
                            <span className="text-stone-500">Pickup: </span>
                            <span className="font-medium text-stone-200">{booking.pickupAddress}</span>
                          </div>
                          <div>
                            <span className="text-stone-500">Drop-off: </span>
                            <span className="font-medium text-stone-200">{booking.destination}</span>
                          </div>
                          <div>
                            <span className="text-stone-500">Schedule: </span>
                            <span className="font-medium text-amber-300">{booking.pickupDate} at {booking.pickupTime}</span>
                          </div>
                          <div>
                            <span className="text-stone-500">Vehicle & Fare: </span>
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2 pt-2 border-t border-stone-800">
                          <div className="flex items-center gap-2 text-[11px] text-stone-400">
                            <span>Target: <strong className="text-amber-400 font-mono">{booking.smsRecipient || ownerPhone}</strong></span>
                            {booking.smsRetryCount ? (
                              <span className="text-amber-300">(Retried {booking.smsRetryCount}x)</span>
                            ) : null}
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-stone-900 hover:bg-stone-800 text-stone-300 text-[11px] border border-stone-800 flex items-center gap-1 cursor-pointer"
                              title="Copy SMS format"
                            >
                              {copiedId === booking.id ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                            ...
                            <button
                              ...
                              className="px-3 py-1 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[11px] font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                            >
                            ...
                            {booking.phone && (
                              <button
                                ...
                                className="px-3 py-1 rounded-lg bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/30 text-[11px] font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                              >
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              <form onSubmit={handleSaveSettings} className="space-y-5 bg-[#14141a] p-6 rounded-2xl border border-stone-800">
                <div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-300 mb-1.5">
                    Designated Business Owner Phone Number <span className="text-amber-400">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-black/70 border border-stone-700 rounded-xl px-4 py-3 text-sm text-white font-mono placeholder-stone-500 focus:outline-none focus:border-amber-500"
                  />
                ...
                <div className="space-y-3 pt-3 border-t border-stone-800">
                  <div className="flex items-center justify-between p-3 rounded-xl bg-black/40 border border-stone-800">
                    <div>
                      ...
                      <span className="text-[11px] text-stone-400">Immediately send the formatted booking template via SMS when customer confirms.</span>
                    </div>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center justify-between p-3 rounded-xl bg-black/40 border border-stone-800">
                    <div>
                      ...
                      <span className="text-[11px] text-stone-400">Notify the rider on their mobile number with reference ID and itinerary details.</span>
                    </div>
                    <input
                      type="checkbox"
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-black font-extrabold text-xs uppercase tracking-wider shadow-lg shadow-amber-500/20 cursor-pointer transition-all"
                >
              ...
              <div className="bg-[#14141a] p-6 rounded-2xl border border-stone-800 space-y-3">
                <h5 className="text-xs uppercase font-bold tracking-wider text-amber-400">
                  Owner SMS Format Preview
                </h5>
                <pre className="bg-black/80 p-4 rounded-xl text-xs text-amber-200 font-mono whitespace-pre-wrap border border-stone-800 leading-relaxed">
{`🚨 NEW RIDE BOOKING
...
Please check the booking dashboard for complete details.`}
          ...
          {activeTab === 'test' && (
            <div className="space-y-5">
              <form onSubmit={handleSendTestSms} className="bg-[#14141a] p-6 rounded-2xl border border-stone-800 space-y-4">
                <div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-black/70 border border-stone-700 rounded-xl px-4 py-3 text-xs text-white placeholder-stone-500 focus:outline-none focus:border-amber-500 resize-none font-mono"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-black font-extrabold text-xs uppercase tracking-wider shadow-lg shadow-emerald-500/20 cursor-pointer disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                >
